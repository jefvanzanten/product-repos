...
function CategoryRow({ category, depth, hasChildren, open }: { readonly category: CategoryDto; readonly depth: number; readonly hasChildren: boolean; readonly open: boolean }): React.ReactNode {
  const target = open
    ? category.parentId
      ? `/admin/product-catalogus?categoryId=${category.parentId}`
      : "/admin/product-catalogus"
    : `/admin/product-catalogus?categoryId=${category.id}`;
  ...
  return (
    <div className={styles.categoryRow} style={{ marginLeft: `${depth}rem` }}>
      ...
      <Link className={styles.editButton} aria-label={`Categorie ${category.name} bewerken`} to={`/admin/product-catalogus/categorieen/${category.id}/bewerken`}><PencilIcon /></Link>
    </div>
  );
}
