...
describe("admin source navigation", () => {
  it.each(["inventory", "calory-tracker"])("accepts the closed source %s", (source) => {
  ...
  it("gives an explicit query source precedence over the fallback cookie", () => {
    const request = new Request(
      "https://apps.example/product-management-admin/product-catalogus?source=calory-tracker",
      { headers: { cookie: "product_management_admin_source=inventory" } },
    );
    expect(resolveAdminSource(request).source).toBe("calory-tracker");
  });
  ...
  it("maps each parsed source to a closed public destination", () => {
    ...
    expect(getAdminSourceDetails("calory-tracker")).toEqual({
