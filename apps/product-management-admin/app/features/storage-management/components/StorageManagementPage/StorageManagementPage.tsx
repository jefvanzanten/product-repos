...
import { AdminLink, useAdminPath } from "../../../../admin-source-context";
import type { LocationActionResult, LocationLoaderData } from "../../location-management.types";
...
export default function StorageManagementPage({ loaderData }: { readonly loaderData: LocationLoaderData }): ReactNode {
  ...
  function restore(node: LocationTreeNode): void {
    ...
    setDialog(null);
    setSubmitted(true);
    fetcher.submit({ _action: "restore", locationId: String(node.id) }, { action: actionTarget, method: "post" });
  }
