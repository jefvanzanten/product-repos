...
function NutritionForm({ errors, onCancel, product, values }: DetailFormProps): React.ReactNode {
  const composition = product.composition;
  return <Form className={styles.editCard} method="post"><input name="intent" type="hidden" value="nutrition" /><input name="productName" type="hidden" value={composition.name} /><input name="categoryId" type="hidden" value={composition.category.id} /><input name="brandId" type="hidden" value={composition.brand?.id ?? ""} /><input name="consumptionType" type="hidden" value={composition.consumptionType} /><h2>Gedeelde voedingswaarden bewerken</h2><p className={styles.impact}>Deze wijziging raakt {composition.productCount} concrete producten.</p><MacroProfileSection errors={errors} profile={composition.macroProfile} values={values} /><ImpactConfirmation count={composition.productCount} /><ProductFormActions onCancel={onCancel} /></Form>;
}
...
function NutritionCard({ product, onEdit }: { readonly product: ConcreteProductDetail; readonly onEdit: () => void }): React.ReactNode {
  ...
  return <section className={styles.card}><div className={styles.cardHeading}><h2>Gedeelde voedingswaarden</h2><span>{product.composition.productCount} gekoppelde producten</span></div>{profile ? <dl className={styles.definitionList}><Row label="Referentie" value={formatReferenceBasis(profile.referenceBasis)} /><Row label="Calorieën" value={profile.caloriesKcal === null ? "-" : `${profile.caloriesKcal} kcal`} /><Row label="Eiwit" value={profile.proteinG === null ? "-" : `${profile.proteinG} g`} /><Row label="Koolhydraten" value={profile.carbohydratesG === null ? "-" : `${profile.carbohydratesG} g`} /><Row label="Vet" value={profile.fatG === null ? "-" : `${profile.fatG} g`} /></dl> : <p className={styles.muted}>Geen macroprofiel toegevoegd.</p>}<button className={styles.secondaryButton} type="button" onClick={onEdit}>{profile ? "Voedingswaarden bewerken" : "Macroprofiel toevoegen"}</button></section>;
}
...
function formatConsumptionType(value: ConcreteProductDetail["composition"]["consumptionType"]): string { return value === "FOOD" ? "Voeding" : value === "DRINK" ? "Drinken" : "Supplement"; }
