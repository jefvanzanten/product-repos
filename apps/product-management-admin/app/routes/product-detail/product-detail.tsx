      ...
      <section className={styles.card}>
        ...
        <section className={styles.packagesSection}>
          ...
          {product.packages.length === 0 ? <p className={styles.muted}>Geen verpakkingen gevonden voor dit product.</p> : product.packages.map((item) => (
            <Link key={item.id} className={styles.packageCard} to={`/product-catalogus/${product.id}/verpakkingen/${item.id}${contextSearch(loaderData.backUrl)}`}>
              <strong>{item.summary}</strong>
              <span>Aantal per verpakking: {item.unitsPerPackage}</span>
            </Link>
          ))}
          <Link className={styles.primaryLink} to={`/product-catalogus/${product.id}/verpakkingen/nieuw${contextSearch(loaderData.backUrl)}`}>Verpakking toevoegen</Link>
        </section>
      </section>
    </main>
  );
