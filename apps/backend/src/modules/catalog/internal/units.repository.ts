...
export function createDrizzleReferenceDataRepository(database: BackendDatabase): ReferenceDataRepository {
  ...
  function findAllPackageTypes(): Array<typeof packageType.$inferSelect> {
    return database.select().from(packageType).orderBy(asc(sql`lower(${packageType.name})`)).all();
  }
