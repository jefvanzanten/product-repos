...
export function productV2Routes(service: ProductV2Service): Hono {
  ...
  router.get("/product-compositions/search", (context) => {
    ...
    return context.json(
      service.searchCompositions(
        query,
        parseLimit(context.req.query("limit"), 20, 100),
      ),
    );
  });
  ...
  router.post("/product-compositions", async (context) => {
    ...
    const result = service.createComposition(input.data);
    return result.ok
      ? context.json(result.value, 201)
      : context.json(result.error, errorStatus[result.error.code]);
  });
  ...
  router.put("/product-compositions/:compositionId", async (context) => {
    ...
    const result = service.updateComposition(
      context.req.param("compositionId"),
      input.data,
    );
    return result.ok
  ...
  router.put(
    ...
    async (context) => {
      ...
      const result = service.updateMacroProfile(
        context.req.param("compositionId"),
        parsed.data,
      );
      return result.ok
        ? context.json(result.value.macroProfile)
        : context.json(result.error, errorStatus[result.error.code]);
    },
  );
  ...
  router.get("/products", (context) => {
    ...
    if (categoryId === "invalid" || archived === "invalid")
      return context.json(
        { code: "VALIDATION_ERROR", message: "Query is invalid" },
        400,
      );
    return context.json(
      service.listProducts({
        ...
        categoryId,
        brandId: context.req.query("brandId"),
        archived,
  ...
  router.post("/products", async (context) => {
    ...
    const result = service.createProduct(input.data);
    return result.ok
      ? context.json(result.value, 201)
      : context.json(result.error, errorStatus[result.error.code]);
  });
  ...
  router.post("/products/:productId/archive", (context) =>
    projectArchiveResult(
      context,
      service.setArchived(context.req.param("productId"), true),
    ),
  );
  router.post("/products/:productId/restore", (context) =>
    projectArchiveResult(
      context,
      service.setArchived(context.req.param("productId"), false),
    ),
  );
...
function projectArchiveResult(
  context: Context,
  result: ReturnType<ProductV2Service["setArchived"]>,
): Response {
  return result.ok
    ? context.json(result.value)
    : context.json(result.error, errorStatus[result.error.code]);
