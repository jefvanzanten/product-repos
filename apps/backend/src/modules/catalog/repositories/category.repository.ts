...
import { category, productComposition } from "../../../db/schema";
...
export function createCategoryRepository(database: BackendDatabase): CategoryRepository {
...
function deleteCategory(id: number): Result<{ readonly id: number }> {
  ...
  const linkedProduct = database.select({ id: productComposition.id }).from(productComposition).where(eq(productComposition.categoryId, id)).get();
