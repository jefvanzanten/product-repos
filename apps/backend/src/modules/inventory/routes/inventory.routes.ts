import {
  addPhysicalInventoryItemsRequestSchema,
  ...
  removePhysicalInventoryItemSchema,
  updatePhysicalInventoryContentSchema,
  updatePhysicalInventoryExpirySchema,
  updatePhysicalInventoryLocationSchema,
  updateProductStockThresholdSchema,
  type InventoryErrorCode,
  type PhysicalInventoryItemDetail,
...
} from "../services/inventory-mutation.service.ts";
import type { InventoryQueryService } from "../services/inventory-query.service.ts";

const listQuerySchema = z
  .object({
    query: z.string().trim().max(200).optional(),
    filter: z.enum(["all", "low-stock", "expiring"]).default("all"),
...
export function inventoryRoutes(dependencies: {
  ...
  router.use("/inventory-items", (context, next) =>
    requireInventorySession(dependencies.sessionResolver, context, next),
  );
  router.use("/inventory-items/*", (context, next) =>
    requireInventorySession(dependencies.sessionResolver, context, next),
  );
  ...
  router.get("/inventory-items/products/search", (context) => {
    ...
    return context.json(
      inventoryProductSearchResultSchema
        .array()
        .parse(
          dependencies.inventoryQueries.searchProducts(
            parsed.data.query,
            parsed.data.limit,
          ),
        ),
  ...
  router.get("/inventory-items/:itemId", (context) => {
    const item = dependencies.inventoryQueries.findItem(
      context.req.param("itemId"),
    );
    return item === null
      ? errorResponse(context, "INVENTORY_ITEM_NOT_FOUND")
      : context.json(physicalInventoryItemDetailSchema.parse(item));
  });
  ...
  router.get("/inventory-items", (context) => {
    ...
    if (
      !parsed.success ||
      (parsed.data.query !== undefined && parsed.data.query.length < 2)
    )
    ...
    const page = dependencies.inventoryQueries.listInventory({
      ...
      today: localToday(context.req.header("X-Browser-Timezone")),
    });
    return context.json(physicalInventoryPageSchema.parse(page));
  });

  router.post("/inventory-items", async (context) => {
    if (!context.get("inventoryIsAdmin"))
      return errorResponse(context, "ADMIN_ROLE_REQUIRED");
    const parsed = addPhysicalInventoryItemsRequestSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!parsed.success) return validationResponse(context);
    const result = dependencies.inventoryMutations.addInventory(
      context.get("inventoryUserId"),
      parsed.data,
    );
    if (!result.ok) return mutationErrorResponse(context, result.error);
    const items = result.value
      .map((id) => dependencies.inventoryQueries.findItem(id))
      .filter((item): item is PhysicalInventoryItemDetail => item !== null)
      .map(toPhysicalItem);
  ...
  router.put("/inventory-items/:itemId/content", async (context) => {
    if (!context.get("inventoryIsAdmin"))
      return errorResponse(context, "ADMIN_ROLE_REQUIRED");
    const parsed = updatePhysicalInventoryContentSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!parsed.success) return validationResponse(context);
    const result = dependencies.inventoryMutations.setContent(
      context.get("inventoryUserId"),
      context.req.param("itemId"),
      parsed.data,
    );
  ...
  router.put("/inventory-items/:itemId/location", async (context) =>
    mutateDetail(
      context,
      updatePhysicalInventoryLocationSchema,
      (body) =>
        dependencies.inventoryMutations.moveItem(
          context.get("inventoryUserId"),
          context.req.param("itemId"),
          body,
        ),
  ...
  router.put("/inventory-items/:itemId/expiry", async (context) =>
    mutateDetail(
      context,
      updatePhysicalInventoryExpirySchema,
      (body) =>
        dependencies.inventoryMutations.setExpiry(
          context.get("inventoryUserId"),
          context.req.param("itemId"),
          body,
        ),
  ...
  router.delete("/inventory-items/:itemId", async (context) => {
    if (!context.get("inventoryIsAdmin"))
      return errorResponse(context, "ADMIN_ROLE_REQUIRED");
    const parsed = removePhysicalInventoryItemSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!parsed.success) return validationResponse(context);
    const result = dependencies.inventoryMutations.removeItem(
      context.get("inventoryUserId"),
      context.req.param("itemId"),
      parsed.data.version,
    );
    return result.ok
      ...
      : mutationErrorResponse(context, result.error);
  });

  router.put(
    "/inventory-items/products/:productId/low-stock-threshold",
    async (context) => {
      if (!context.get("inventoryIsAdmin"))
        return errorResponse(context, "ADMIN_ROLE_REQUIRED");
      const parsed = updateProductStockThresholdSchema.safeParse(
        await context.req.json().catch(() => null),
      );
      if (!parsed.success) return validationResponse(context);
      const result = dependencies.inventoryMutations.setThreshold(
        context.req.param("productId"),
        parsed.data,
      );
      return result.ok
        ? context.json(productStockThresholdSchema.parse(result.value))
        : mutationErrorResponse(context, result.error);
    },
  );
...
async function mutateDetail<T>(
  ...
  dependencies: { readonly inventoryQueries: InventoryQueryService },
): Promise<Response> {
  if (!context.get("inventoryIsAdmin"))
    return errorResponse(context, "ADMIN_ROLE_REQUIRED");
  ...
  return result.ok
    ? detailResponse(context, dependencies.inventoryQueries, result.value)
    : mutationErrorResponse(context, result.error);
}
...
function localToday(timezone: string | undefined): string {
  ...
  try {
    new Intl.DateTimeFormat("en-CA", { timeZone: safeTimezone }).format();
  } catch {
    safeTimezone = "UTC";
  }
...
function errorResponse(
  ...
  message: string = errorDetails[code].message,
): Response {
  return context.json(
    inventoryErrorResponseSchema.parse({ code, message }),
...
const errorDetails = {
  ...
  REFERENCE_NOT_FOUND: {
    status: 400,
    message: "A referenced value is invalid",
  },
  ADMIN_ROLE_REQUIRED: {
    status: 403,
  ...
  PRODUCT_NOT_FOUND: { status: 404, message: "Product was not found" },
  INVENTORY_ITEM_VERSION_CONFLICT: {
    status: 409,
    message: "Inventory item has changed",
  },
  PRODUCT_ARCHIVED: { status: 409, message: "Product is archived" },
  PRODUCT_CONTENT_UNKNOWN: {
    status: 409,
    message: "Product content is unknown",
  },
  AMOUNT_EXCEEDS_PRODUCT_CONTENT: {
    status: 409,
    message: "Amount exceeds product content",
  },
