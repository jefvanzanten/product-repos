...
const packageSearchQuerySchema = z
  .object({
...
export function calorieTrackerRoutes(dependencies: {
  ...
  const { consumptionLogs, nutritionSummary, packageSelection, unifiedSearch } =
    dependencies;
  const router = new Hono<CalorieTrackerEnvironment>();

  router.use("*", (context, next) =>
    requireCalorieTrackerSession(dependencies.sessionResolver, context, next),
  );
  ...
  router.get("/products/search", (context) => {
    ...
    const result = packageSelection.searchPackages(
  ...
  router.get("/search", (context) => {
    ...
    if (!parsed.success) return validationResponse(context);
    const result = unifiedSearch.search(
      context.get("calorieTrackerUserId"),
      parsed.data.query,
      parsed.data.limit,
    );
  ...
  router.post("/logs", async (context) => {
    ...
    const body = createConsumptionLogSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!body.success) return validationResponse(context);
    const result = consumptionLogs.createLog(
      context.get("calorieTrackerUserId"),
      timezone.value,
      body.data,
    );
    ...
    return context.json(
      result.value.log,
      result.value.state === "created" ? 201 : 200,
    );
  });
  ...
  router.get("/logs/:logId", (context) => {
    ...
    if (!logId.success)
      return context.json(
        { code: "LOG_NOT_FOUND", message: "Log not found" },
        404,
      );
    const result = consumptionLogs.getLog(
      context.get("calorieTrackerUserId"),
      logId.data,
    );
    return result.ok
  ...
  const replaceLog = async (
    ...
    const timezone = parseTimezoneHeader(context);
    if (!timezone.ok) return timezone.response;
    const body = updateConsumptionLogSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!body.success) return validationResponse(context);
    const result = consumptionLogs.updateLog(
      context.get("calorieTrackerUserId"),
      logId.data,
      timezone.value,
      body.data,
  ...
  router.delete("/logs/:logId", (context) => {
    ...
    const result = consumptionLogs.deleteLog(
      context.get("calorieTrackerUserId"),
      logId.data,
    );
    return result.ok
  ...
  router.post("/logs/:logId/restore", (context) => {
    ...
    const result = consumptionLogs.restoreLog(
      context.get("calorieTrackerUserId"),
      logId.data,
    );
    return result.ok
  ...
  router.get("/goals", (context) => {
    const result = nutritionSummary.getGoals(
      context.get("calorieTrackerUserId"),
    );
    return result.ok
      ? context.json(nutritionGoalSchema.parse(result.value))
      : errorResponse(context, result);
  });

  router.put("/goals", async (context) => {
    const body = upsertNutritionGoalSchema.safeParse(
      await context.req.json().catch(() => null),
    );
    if (!body.success) return validationResponse(context);
    const result = nutritionSummary.replaceGoals(
      context.get("calorieTrackerUserId"),
      body.data,
    );
    return result.ok
...
function parseTimezoneHeader(
  context: Context<CalorieTrackerEnvironment>,
):
  | { readonly ok: true; readonly value: string }
  | { readonly ok: false; readonly response: Response } {
  const schemaResult = browserTimezoneSchema.safeParse(
    context.req.header("X-Browser-Timezone"),
  );
  if (!schemaResult.success)
    return {
      ok: false,
      response: validationResponse(context, "Browser timezone is required"),
    };
  const parsed = parseTimezone(schemaResult.data);
  if (!parsed.ok)
    return { ok: false, response: context.json(parsed.error, 400) };
...
function parseDateBoundary<T extends z.ZodType<{ readonly date: string }>>(
  ...
  | {
      readonly ok: true;
      readonly value: { readonly query: z.infer<T>; readonly timezone: string };
    }
  | { readonly ok: false; readonly response: Response } {
  const timezone = parseTimezoneHeader(context);
  ...
  if (!query.success)
    return { ok: false, response: validationResponse(context) };
...
function errorResponse<T>(
  context: Context<CalorieTrackerEnvironment>,
  result: Extract<CalorieTrackerResult<T>, { readonly ok: false }>,
): Response {
