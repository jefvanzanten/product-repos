...
type JoinedConsumptionLogRow = {
  ...
  readonly inputMode: "FULL_PRODUCT" | "PRODUCT_PORTION" | "CONTENT_UNIT" | null;
  readonly inputUnitTypeId: number | null;
...
export function createConsumptionLogRepository(db: BackendDatabase): ConsumptionLogRepository {
  ...
  function toRecord(row: JoinedConsumptionLogRow): ConsumptionLogRecord | undefined {
    ...
    if (row.productId === null || row.productQuantity === null || row.inputMode === null) return undefined;
    return {
      id: row.id,
      userId: row.userId,
      type: "PRODUCT",
