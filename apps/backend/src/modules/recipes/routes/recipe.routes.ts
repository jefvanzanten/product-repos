...
import type { RecipeResult, RecipeService } from "../services/recipe.service.ts";
...
export function recipeRoutes(dependencies: {
  ...
  router.use("*", (context, next) => resolveOptionalSession(dependencies.sessionResolver, context, next));
  ...
  router.get("/recipes/users/:userId", (context) => {
    ...
    const result = dependencies.recipes.listForUser(context.get("recipeViewerUserId"), userId.data, query.data);
    if (!result.ok) return errorResponse(context, result);
    setReadCacheHeaders(context);
    return context.json(recipePageSchema.parse(result.value));
  });
  ...
  router.get("/recipes/users/:userId/:dishId", (context) => {
    ...
    const result = dependencies.recipes.getRecipe(context.get("recipeViewerUserId"), userId.data, dishId.data);
    if (!result.ok) return errorResponse(context, result);
    setReadCacheHeaders(context);
  ...
  router.post("/recipes", requireSession, async (context) => {
    const body = createRecipeSchema.safeParse(await readJson(context));
    if (!body.success) return validationResponse(context);
    const result = dependencies.recipes.createRecipe(context.get("recipeViewerUserId")!, body.data);
    return result.ok ? context.json(recipeDetailSchema.parse(result.value), 201) : errorResponse(context, result);
  });

  router.put("/recipes/:dishId", requireSession, async (context) => {
    const dishId = uuidSchema.safeParse(context.req.param("dishId"));
    const body = updateRecipeSchema.safeParse(await readJson(context));
    if (!dishId.success || !body.success) return validationResponse(context);
    const result = dependencies.recipes.updateRecipe(context.get("recipeViewerUserId")!, dishId.data, body.data);
    return result.ok ? context.json(recipeDetailSchema.parse(result.value)) : errorResponse(context, result);
  });

  router.post("/recipes/:dishId/archive", requireSession, (context) => {
    const dishId = uuidSchema.safeParse(context.req.param("dishId"));
    if (!dishId.success) return neutralNotFound(context);
    const result = dependencies.recipes.archiveRecipe(context.get("recipeViewerUserId")!, dishId.data);
    return result.ok ? context.json(recipeArchiveResultSchema.parse(result.value)) : errorResponse(context, result);
  });

  router.post("/recipes/:dishId/restore", requireSession, (context) => {
    const dishId = uuidSchema.safeParse(context.req.param("dishId"));
    if (!dishId.success) return neutralNotFound(context);
    const result = dependencies.recipes.restoreRecipe(context.get("recipeViewerUserId")!, dishId.data);
    return result.ok ? context.json(recipeDetailSchema.parse(result.value)) : errorResponse(context, result);
  });

  router.get("/recipes/products/search", requireSession, (context) => {
    const query = productSearchQuerySchema.safeParse(Object.fromEntries(new URL(context.req.url).searchParams.entries()));
    if (!query.success) return validationResponse(context);
    const result = dependencies.recipes.searchProducts(query.data.query, query.data.limit);
    return result.ok ? context.json(recipeProductSearchResultSchema.array().parse(result.value)) : errorResponse(context, result);
  });

  router.get("/recipes/products/:productId/input-units", requireSession, (context) => {
    const productId = uuidSchema.safeParse(context.req.param("productId"));
    if (!productId.success) return context.json({ code: "REFERENCE_NOT_FOUND", message: "Product not found" }, 400);
    const result = dependencies.recipes.getInputOptions(productId.data);
    return result.ok ? context.json(recipeIngredientInputOptionsSchema.parse(result.value)) : errorResponse(context, result);
  });
...
async function requireSession(context: Context<RecipeEnvironment>, next: Next): Promise<Response | void> {
  if (context.get("recipeViewerUserId") === undefined) return context.json({ code: "UNAUTHENTICATED", message: "Authentication is required" }, 401);
  await next();
}
...
function parseListQuery(context: Context<RecipeEnvironment>) {
  return listQuerySchema.safeParse(Object.fromEntries(new URL(context.req.url).searchParams.entries()));
}

/** Read JSON without leaking parser exceptions through the adapter. */
async function readJson(context: Context<RecipeEnvironment>): Promise<unknown> {
...
function setReadCacheHeaders(context: Context<RecipeEnvironment>): void {
  ...
  context.header("Cache-Control", context.get("recipeViewerUserId") === undefined ? "public, max-age=60, stale-while-revalidate=300" : "private, no-store");
}

/** Render a strict request validation response. */
function validationResponse(context: Context<RecipeEnvironment>): Response {
  return context.json({ code: "VALIDATION_ERROR", message: "Request is invalid" }, 400);
}

/** Render the shared neutral recipe-not-found response. */
function neutralNotFound(context: Context<RecipeEnvironment>): Response {
  return context.json({ code: "DISH_NOT_FOUND", message: "Recipe not found" }, 404);
}

/** Translate an expected recipe failure into a documented HTTP response. */
function errorResponse<T>(context: Context<RecipeEnvironment>, result: Extract<RecipeResult<T>, { readonly ok: false }>): Response {
  if (result.error.code === "INTERNAL_ERROR") {
    const correlationId = crypto.randomUUID();
    console.error("Recipe projection invariant failure", { correlationId, operation: `${context.req.method} ${new URL(context.req.url).pathname}` });
