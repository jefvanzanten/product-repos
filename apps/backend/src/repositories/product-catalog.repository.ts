...
import { findAllBrands, findBrandById, type BrandRow } from "./brands.repository";
import { findAllCategories, findCategoryById, findCategoryPath, formatCategoryPath, type CategoryRow } from "./category.repository";
import { findProductPackageFullRows, toProductPackageDto } from "./product-packages.repository";
import { compareProductRows, displayProductName, findAllProductRows, type ProductRow } from "./products.repository";

export function browseProductCatalog(input: { readonly categoryId?: number; readonly brandId?: string; readonly limit: number }): Result<CatalogBrowseResponse> {
...
export function searchProductCatalog(input: {
