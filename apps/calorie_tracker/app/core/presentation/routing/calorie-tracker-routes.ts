...
/** Build the statistics route and omit URL state that matches its defaults. */
export function statisticsPath(date: string, type?: ConsumptionTypeFilter, today?: string): string {
  const search = new URLSearchParams();
  if (date !== today) search.set("date", date);
  if (type !== undefined && type !== "all") search.set("type", type);
