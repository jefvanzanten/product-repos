...
import { canonicalizeTrackerUrl } from "../../../routing/tracker-url-state";
import { getTodayDate } from "../../../../domain/dates-and-timezones";
import { formatLocalDate } from "../../../formatting/dates";
 ...
 *
 * @param nextDate - The nextDate value.
 * @returns Nothing.
 */
  function selectHeaderDate(nextDate: string): void {
    const nextParameters = new URLSearchParams();
    if (location.pathname === "/logs" || nextDate !== today) nextParameters.set("date", nextDate);
    if (location.pathname === "/logs" || (type !== "all" && parameters.has("type"))) {
      nextParameters.set("type", type);
    }
  ...
  return (
    <header className={styles.trackerHeader}>
      ...
      <nav className={className} aria-label="Calorie Tracker">
        <NavLink to={statisticsPath(date, type, today)} end>Caloriestatistieken</NavLink>
