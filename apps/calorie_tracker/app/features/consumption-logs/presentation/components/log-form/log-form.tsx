...
import {
  createExistingUnit,
  createExistingUnitKey,
  createUnitKey,
} from "../../../domain/log-form-units";
import type {
  LogFormActionResult,
  LogFormMode,
} from "../../types/log-form.types";
import {
  formatTimeInTimezone,
  toFormMomentInTimezone,
} from "../../formatting/consumption-moment";
...
type ProductUnitsData =
  | {
      ...
      readonly units: ReadonlyArray<AvailableInputUnit>;
    }
  | {
      readonly ok: false;
...
export function LogForm({
  ...
  const editDish =
    mode.tag === "Edit" && mode.log.type === "DISH" ? mode.log : null;
  const initialSelection: ConsumableSelection | null =
    mode.tag === "Edit"
      ? mode.log.type === "DISH"
  ...
  const [selection, setSelection] = useState<ConsumableSelection | null>(
    initialSelection,
  );
  const [quantity, setQuantity] = useState(
    mode.tag === "Edit" ? mode.log.quantity.replace(".", ",") : "1",
  );
  const initialMoment =
    mode.tag === "Edit"
      ? {
          date: mode.log.localDate,
          time: formatTimeInTimezone(mode.log.consumedAt, mode.log.timezone),
        }
      : toFormMomentInTimezone(new Date().toISOString(), timezone);
  const [selectedDate, setSelectedDate] = useState(
    mode.tag === "Edit" ? initialMoment.date : date,
  );
  const [time, setTime] = useState(initialMoment.time);
  const [selectedUnitKey, setSelectedUnitKey] = useState<string | null>(
    mode.tag === "Edit" && mode.log.type === "PRODUCT"
      ? createExistingUnitKey(mode.log)
      : null,
  );
  ...
  const selectedProduct =
    selection !== null && selection.kind === "PRODUCT" ? selection.value : null;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void loadSearchResults(
        `/consumable-lookup?${new URLSearchParams({ query: normalizedQuery })}`,
      );
    }, 300);
    return () => window.clearTimeout(timer);
  }, [editDish, loadSearchResults, normalizedQuery]);

  const selectedOriginalProduct =
    mode.tag === "Edit" &&
    mode.log.type === "PRODUCT" &&
    selectedProduct !== null &&
    shouldIncludeLegacyInputUnit(
      selectedProduct.productId,
      mode.log.product.productId,
    );
  const selectedOriginalArchived =
  ...
  useEffect(() => {
    ...
    const logFilter =
      savedLog.type === "DISH"
        ? "food"
        : (savedLog.product.consumptionType?.toLowerCase() ?? "all");
    const visible = type === "all" || type === logFilter;
    const message = visible
      ? "Log opgeslagen."
      : "Log opgeslagen, maar niet zichtbaar binnen het actieve filter.";
  ...
  const lookupMatches =
    lookupData?.ok === true && lookupData.query === normalizedQuery;
  const rawResults =
    searchMode.tag === "Recent"
      ? lookupData?.ok === true && lookupData.query === null
        ? lookupData.results
        : initialResults
  ...
  const results =
    mode.tag === "Edit"
      ...
      : rawResults;
  const lookupFailed =
    lookupData?.ok === false && lookupData.query === normalizedQuery;
  const fetchedUnits =
    unitsFetcher.data?.ok === true &&
    selectedProduct !== null &&
  ...
  const existingUnit =
    ...
    mode.tag === "Edit" &&
    mode.log.type === "PRODUCT"
      ? createExistingUnit(mode.log)
      : null;
  const includesExistingUnit =
    existingUnit === null ||
    fetchedUnits.some(
      (unit) => createUnitKey(unit) === createUnitKey(existingUnit),
    );
  const availableUnits =
    existingUnit !== null && !includesExistingUnit
      ? [existingUnit, ...fetchedUnits]
      : fetchedUnits;
  const unitsLoaded =
    selectedOriginalArchived ||
    (unitsFetcher.data?.ok === true &&
      selectedProduct !== null &&
      unitsFetcher.data.productId === selectedProduct.productId);
  const unitKey =
    selectedProduct !== null && unitsLoaded && availableUnits.length > 0
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>): void {
    ...
    if (
      submission.requiresQuantityConfirmation &&
      !window.confirm("Deze hoeveelheid is uitzonderlijk hoog. Toch opslaan?")
    )
      return;
   ...
   * @returns Nothing.
   */
  function selectResult(result: ConsumableOption): void {
    setSelection(
      result.kind === "DISH"
        ? {
            kind: "DISH",
  ...
  const formError =
    validationError ??
    (mutationFetcher.data?.ok === false ? mutationFetcher.data.error : null);
  const unitsFailed =
    unitsFetcher.data?.ok === false &&
    selectedProduct !== null &&
    unitsFetcher.data.productId === selectedProduct.productId;
  ...
  return (
    <ModalDialog
      title={formTitle}
      onClose={() => void navigate(closePath)}
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.fields}>
            ...
            <label className={styles.fullField}>
              ...
              <span className={styles.inputWithIcon}>
                ...
                <input
                  ...
                  placeholder="Zoek op product, merk of gerecht"
                />
              </span>
            </label>
          )}
          ...
          {editDish === null && (
            <SearchResultList
              ...
              isPending={
                searchMode.tag === "Search" &&
                (!lookupMatches || searchFetcher.state !== "idle")
              }
              failed={lookupFailed}
              results={results}
              selectedKey={
                selection === null
                  ...
                  : selection.kind === "PRODUCT"
                    ? `product:${selection.value.productId}`
                    : `dish:${selection.value.id}`
              }
              editLog={
                mode.tag === "Edit" && mode.log.type === "PRODUCT"
                  ? mode.log
              ...
              onRetry={() => {
                void (searchMode.tag === "Search"
                  ? loadSearchResults(
                      `/consumable-lookup?${new URLSearchParams({ query: searchMode.query })}`,
                    )
          ...
          {editDish !== null && (
            <p className={styles.fullField}>
              Geselecteerd gerecht: <strong>{editDish.dish.name}</strong>
            </p>
          )}
            ...
            <div ref={quantityFieldsRef} className={styles.quantityFields}>
              {selection.kind === "PRODUCT" ? (
                <QuantityFields
                  ...
                  unitsDisabled={
                    !selectedOriginalArchived && (!unitsLoaded || unitsFailed)
                  }
                  ...
                  onRetryUnits={() =>
                    void loadUnits(
                      `/product-input-units/${selection.value.productId}`,
                    )
                  }
                />
              ) : (
          ...
          {formError !== null && (
            <div className={styles.error} role="alert">
              {formError}
              {mode.tag === "Edit" && formError.includes("intussen") && (
                <button type="button" onClick={() => window.location.reload()}>
                  Actuele data herladen
                </button>
              )}
            </div>
          )}
        ...
        <footer className={`${styles.footer} ${styles.singleActionFooter}`}>
          <button
            ...
            disabled={mutationFetcher.state !== "idle"}
          >
