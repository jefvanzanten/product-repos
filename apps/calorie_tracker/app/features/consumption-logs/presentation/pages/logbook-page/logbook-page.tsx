...
import { newLogPath } from "../../../../../core/presentation/routing/calorie-tracker-routes";
...
export function LogbookPage({ loaderData }: { readonly loaderData: LogbookLoaderData }): ReactNode {
  ...
  useEffect(() => {
    if (content?.tag !== "Ready" || routeState === null) return;
  ...
  return (
    <main className={styles.page}>
      <section className={styles.panel}>
        <Link className={`${styles.desktopAction} ct-primary`} to={newLogPath(routeState)}><Icon name="add" />Log toevoegen</Link>
        ...
        <div className={styles.list} ref={listRef}>
          ...
          {content?.tag === "EmptyDate" && <StatusPanel title="Nog geen consumpties" message={`Er zijn geen logs op ${formatLocalDate(routeState.date, "compact")}.`} />}
          {content?.tag === "EmptyFilter" && <StatusPanel title="Geen resultaten binnen dit filter" message="Kies een ander type of toon alle consumpties." action={<button type="button" className="ct-secondary" onClick={() => setParameters({ date: routeState.date, type: "all" })}>Alles tonen</button>} />}
          {content?.tag === "Ready" && content.items.map((item) => <LogItem key={item.id} item={item} routeState={routeState} />)}
        </div>
