import type { NutritionGoal } from "@product-repos/contracts/calorie-tracker";
...
import type { GoalDraft } from "../../utils/goals-draft";
...
export function StatisticsPage({ loaderData }: { readonly loaderData: StatisticsLoaderData }): ReactNode {
  ...
  useEffect(() => {
    if (timezone === null || routeState === null) return;
    const today = getTodayInTimezone(timezone);
    if (routeState.date !== today) return;
    const delay = millisecondsUntilLocalDateChanges(timezone, today, new Date());
    const timer = window.setTimeout(() => {
      const next = new URLSearchParams({ date: getTodayInTimezone(timezone) });
      if (parameters.has("type")) next.set("type", routeState.type);
      setParameters(next, { replace: true });
    }, delay);
    return () => window.clearTimeout(timer);
  }, [parameters, routeState, setParameters, timezone]);
