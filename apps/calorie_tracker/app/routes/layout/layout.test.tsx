 ...
 *
 * @returns Authenticated layout loader data.
 */
...
async function verifyActiveTrackerTabOnLogbook(): Promise<void> {
  ...
  expect(within(primaryNavigation).getByRole("link", { name: "Calorie Tracker" })).toHaveAttribute("aria-current", "page");
}
...
function registerLayoutTests(): void {
  it("keeps Calorie Tracker active while viewing the consumption logbook", verifyActiveTrackerTabOnLogbook);
}
