...
export type LocalConsumptionMomentError = {
  readonly _tag: "InvalidMoment" | "AmbiguousMoment" | "FutureMoment";
};
...
export function parseLocalConsumptionMoment(
  ...
  if (fields === null) return { _tag: "Failure", error: { _tag: "InvalidMoment" } };
  ...
  if (candidates.length === 0) return { _tag: "Failure", error: { _tag: "InvalidMoment" } };
  if (candidates.length > 1) return { _tag: "Failure", error: { _tag: "AmbiguousMoment" } };
  const instant = candidates[0];
  if (instant === undefined) return { _tag: "Failure", error: { _tag: "InvalidMoment" } };
  if (instant > now.getTime()) return { _tag: "Failure", error: { _tag: "FutureMoment" } };
  return { _tag: "Success", value: new Date(instant).toISOString() };
}
...
export function parseEditedConsumptionMoment(
  ...
  if (original !== null && dateValue === original.date && timeValue === original.time) {
    return Date.parse(original.consumedAt) <= now.getTime()
      ? { _tag: "Success", value: original.consumedAt }
      : { _tag: "Failure", error: { _tag: "FutureMoment" } };
  }
