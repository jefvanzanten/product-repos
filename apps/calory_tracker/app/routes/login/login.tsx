...
import {
  parseCaloryTrackerReturnPath,
  toCaloryTrackerRedirectPath,
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const returnTo = parseCaloryTrackerReturnPath(requestUrl.searchParams.get("returnTo"));
  const result = await lookupSession(request);
  if (result._tag === "Authenticated") {
    throw redirect(toCaloryTrackerRedirectPath(returnTo));
  }
