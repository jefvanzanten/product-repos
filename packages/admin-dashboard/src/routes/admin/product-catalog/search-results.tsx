...
export function SearchResults({ search }: { readonly search: CatalogSearchResponse }): React.ReactNode {
  ...
  return (
    <div className={styles.searchResults}>
      ...
      {search.brands.length > 0 ? <ResultGroup title="Merken">{search.brands.map((brand) => <Link key={brand.id} className={styles.resultRow} to={`/admin/product-catalogus?brandId=${brand.id}`}><strong>{brand.name}</strong><span>{brand.productCount} producten</span></Link>)}</ResultGroup> : null}
      {search.categories.length > 0 ? <ResultGroup title="Categorieën">{search.categories.map((category) => <Link key={category.id} className={styles.resultRow} to={`/admin/product-catalogus?categoryId=${category.id}`}><strong>{category.path}</strong><span>{category.productCount} producten</span></Link>)}</ResultGroup> : null}
    </div>
  );
}
